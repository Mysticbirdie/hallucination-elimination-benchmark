#!/usr/bin/env python3
"""
Hallucination Elimination Benchmark — Ollama Runner
====================================================
Runs the 222-question Rome 110 CE benchmark against any local model via Ollama.
Works with Mistral, Bielik, LLaMA, Qwen, or any GGUF model loaded in Ollama.

Usage:
    python run_ollama.py --model mistral:instruct
    python run_ollama.py --model bielik:latest --triad
    python run_ollama.py --model llama3:8b --triad --no-resume

Requirements:
    - Ollama running locally (https://ollama.ai)
    - Model pulled in Ollama: ollama pull mistral:instruct
...
def build_raw_system(char_map, char_id=None):
    """Minimal system prompt — no cultural context injected."""
    if char_id and char_id in char_map:
        c = char_map[char_id]
        return (
...
# ── Category-specific question wrapper ───────────────────────────────────────
def wrap_question(question, category, char_id=None, char_map=None):
    """Apply category-specific prompting strategy."""
    if category == "ANACHRONISM_DETECTION":
        ...
        if char_id and char_map and char_id in char_map:
            c = char_map[char_id]
        ...
        prefix = (
            "Think through this carefully using Roman law, custom, and social norms of 110 CE:\n\n"
        )
        return prefix + question
...
# ── Ollama call ───────────────────────────────────────────────────────────────
def call_ollama(model, system_prompt, user_question, temperature=0.0, timeout=120):
    try:
        resp = requests.post(
            OLLAMA_URL,
            json={
...
def save_results(data, filepath):
    ...
    jf = sum(1 for d in details if d["verdict"] == "JUDGE_FAILED")
    total = len(details)
    cats = {}
    for d in details:
        cat = d["category"]
...
# ── Main ──────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="Run hallucination benchmark via Ollama")
    parser.add_argument("--model", required=True, help="Ollama model name (e.g. mistral:instruct)")
    parser.add_argument("--triad", action="store_true", help="Use Triad Engine (cultural context injection)")
    ...
    try:
        ...
        if args.model not in models:
            print(f"ERROR: '{args.model}' not found in Ollama.")
            print(f"Available models: {', '.join(models) if models else 'none'}")
            print(f"Pull the model first: ollama pull {args.model}")
            sys.exit(1)
    except Exception as e:
    ...
    for q in questions:
        ...
        answer = call_ollama(args.model, system, user_msg, args.temperature)
